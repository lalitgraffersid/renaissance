...
class Advanced_Tabs extends Powerpack_Widget {
	...
	public function get_title() {
		return __( 'Advanced Tabs', 'powerpack' );
	}
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'raw'             => sprintf( __( '%1$s Watch Video Overview %2$s', 'powerpack' ), '<a href="https://www.youtube.com/watch?v=B8pZWoWv6qM&list=PLpsSO_wNe8Dz4vfe2tWlySBCCFEgh1qZj" target="_blank" rel="noopener">', '</a>' ),
		...
		$this->add_control(
			...
			[
				...
				'default'   => '#000000',
				'selectors' => [
					'{{WRAPPER}} .pp-tab-active i' => 'color: {{VALUE}}',
		...
		$this->add_control(
			...
			[
				...
				'default'   => '#000000',
				'selectors' => [
					'{{WRAPPER}} .pp-tab-active span' => 'color: {{VALUE}}',
		...
		$this->add_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .pp-tab-active' => 'border-color: {{VALUE}}',
	...
	public function render_tab_title_icon( $item ) {
		...
		if ( ! empty( $item['icon'] ) || ( ! empty( $item['selected_icon']['value'] ) && $is_new ) ) {
			...
			<?php
			if ( $is_new || $migrated ) {
				Icons_Manager::render_icon( $item['selected_icon'], [ 'class' => 'pp-advanced-tabs-icon-' . $settings['icon_position'], 'aria-hidden' => 'true' ] );
			} else { ?>
				<i class="<?php echo esc_attr( $item['icon'] . ' pp-advanced-tabs-icon-' . $settings['icon_position'] ); ?>" aria-hidden="true"></i>
			<?php } ?>
			</span>
			<?php
		}
