	...
	class Jet_Engine_Forms_File_Upload {
		...
		public function get_result_html( $field = array(), $files = array() ) {
			...
			foreach ( $files as $file ) {
				...
				$result .= sprintf( $format, $file['url'], $attachment, $result_format );
		...
		public function process_upload( $files = false, $settings = array() ) {
			...
			if ( count( $files ) > $settings['max_files'] ) {
				wp_send_json_error( __( 'Maximum upload files limit is reached', 'jet-engine' ) );
			}
			...
			foreach ( $files as $file ) {
				...
				if ( ! $file['size'] > $settings['max_size'] ) {
					wp_send_json_error( __( 'Upload max size exceeded', 'jet-engine' ) );
				}

				if ( ! empty( $settings['mime_types'] ) && ! in_array( $file['type'], $settings['mime_types'] ) ) {
					wp_send_json_error( __( 'File type is not allowed', 'jet-engine' ) );
				}
		...
		public function register_assets() {
			...
			wp_localize_script( 'jet-engine-file-upload', 'JetEngineFileUploadConfig', array(
				...
				'errors'          => array(
					'upload_limit' => __( 'Maximum upload files limit is reached', 'jet-engine' ),
					'file_type'    => __( 'File type is not supported', 'jet-engine' ),
					'file_size'    => __( 'Maximum upload file size is exceeded', 'jet-engine' ),
				),
			) );
